...
const WordleAI: React.FC = () => {
  ...
  const onKeyPress = (key: string) => {
    ...
    if (key === 'ENTER') {
      if (currentLetter === 5) submitGuess();
      return;
    }
  ...
  const submitGuess = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (target[i] === char) status = 'correct';
      else if (target.includes(char)) status = 'present';

      if (newUsedLetters[char] !== 'correct') newUsedLetters[char] = status;
    }
  ...
  const getStatusColor = (status: Status) => {
    switch (status) {
      case 'correct': return 'bg-emerald-600 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.4)]';
      case 'present': return 'bg-amber-500 border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.4)]';
      case 'absent': return 'bg-slate-800 border-slate-700';
  ...
  return (
    <div className="flex flex-col items-center h-full p-2 space-y-6 pt-6 bg-slate-950">
      <style>{`.letter-glow { text-shadow: 0 0 10px rgba(255,255,255,0.3); }`}</style>
      
      <div className="grid grid-rows-6 gap-2.5">
        {guesses.map((row, r) => (
          <div key={r} className="grid grid-cols-5 gap-2.5">
            {[0, 1, 2, 3, 4].map((c) => (
              <div key={c} className={`w-12 h-14 border-2 flex items-center justify-center text-2xl font-black rounded-xl transition-all duration-500 text-white letter-glow ${getStatusColor(getLetterStatus(r, c))} ${r === currentGuessIndex && c === currentLetter ? 'border-indigo-400 ring-2 ring-indigo-400/20' : ''}`}>
                {row[c] || ''}
              </div>
            ))}
          </div>
      ...
      {gameState !== 'playing' && (
        <div className="text-center bg-slate-900/95 backdrop-blur-xl p-6 rounded-[2rem] border border-white/10 shadow-2xl w-full max-w-[320px] animate-in zoom-in duration-500 z-50">
          ...
          <button onClick={initGame} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-xs tracking-[0.2em] active:scale-95 transition-all shadow-xl">
            NUEVA PALABRA
          </button>
        </div>
      )}
      ...
      <div className="w-full max-w-[420px] space-y-2 pb-8 mt-auto px-1">
        {[
          ...
          <div key={i} className="flex justify-center gap-1">
            {row.map((key) => (
              <button key={key} onClick={() => onKeyPress(key)} className={` ${key.length > 1 ? 'px-3 text-[10px]' : 'w-8 h-12 text-sm'} rounded-xl flex items-center justify-center font-black transition-all active:scale-90 border-b-4 uppercase text-white letter-glow ${usedLetters[key] === 'correct' ? 'bg-emerald-600 border-emerald-800' : usedLetters[key] === 'present' ? 'bg-amber-500 border-amber-700' : usedLetters[key] === 'absent' ? 'bg-slate-800 border-slate-950' : 'bg-slate-700 border-slate-800'}`}>
